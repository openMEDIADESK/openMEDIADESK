...
public class PinPicViewController extends AbstractThumbnailViewController {
    ...
    protected String getListViewJsp(HttpServletRequest request) {
        ...
        Pinpic pinpic = (Pinpic)request.getAttribute("pin");
        final int defaultView = pinpic.getDefaultview();
        switch (defaultView) {
        ...
        return super.getListViewJsp(request);    //To change body of overridden methods use File | Settings | File Templates.
    }
    ...
    protected List loadThumbnailImageList(int sortBy, int orderBy, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) {
        ...
        if (httpServletRequest.getParameter("pinid")!=null) {
            ...
            if (httpServletRequest.getSession().getAttribute("pinid")!=null) {
                ...
                //keine pin angegeben...
                //zu /pin redirecten:
                logger.debug("Redirect:  "+httpServletResponse.encodeRedirectURL("/pinpic"));
                try {
                    httpServletResponse.sendRedirect(
                        httpServletResponse.encodeRedirectURL("pin")
                );
        ...
        LngResolver lngResolver = new LngResolver();
        pinpicService.setUsedLanguage(lngResolver.resolveLng(httpServletRequest));
        List imageList = pinpicService.getPinpicImages(pinId);
        System.out.println("Thumbnail-ImageList: "+imageList.size());
        return imageList;
        */
    }

    protected List loadAllImageList(HttpServletRequest request, HttpServletResponse response) throws LoadThumbnailException {
        return loadImageList(-1, -1, request, response); 
    }

    private List loadImageList(int sortBy, int orderBy, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) {
        int pinId = -1;
    ...
    protected MediaMenu mediaMenuBaker(User user, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (getUser(request).getRole()>=User.ROLE_PINEDITOR) {
            mediaMenu.setSelectionFromCategory(true);

            //Aktionsmenü
            mediaMenu.setAction(true);
            mediaMenu.setActionEditPin(true);
            mediaMenu.setActionUpload(true);
            mediaMenu.setActionPinLink(true);
        }

        if (isUploadEnabled(this.getPin(request), request)) {
            mediaMenu.setVisible(true);
            mediaMenu.setAction(true);
            mediaMenu.setActionUpload(true);
        }

        mediaMenu.setDownloadPin(true);



        /*
        mediaMenu.setDelete(true);
        mediaMenu.setDeleteFromPin(true);
        //mediaMenu.setDeleteFromDB(true);
        //mediaMenu.setDeleteFromFolder(true);
        mediaMenu.setSelection(true);
        //mediaMenu.setSelectionAsFolderImage(true);
        mediaMenu.setSelectionCopy(true);
        ...
        request.setAttribute("pinmenu",true);
