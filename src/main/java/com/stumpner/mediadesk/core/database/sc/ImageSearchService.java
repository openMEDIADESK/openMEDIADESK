...
import com.stumpner.mediadesk.search.ImageVersionSearchProperty;
 ...
 */
public class ImageSearchService extends ImageVersionService {

    static Logger logger = Logger.getLogger(ImageSearchService.class);
    ...
    public SearchResult getImageQuery(KeywordSearchProperty keywordSearchProperty, int page, int itemsPerPage, User user) {
        ...
        if (keywordSearchProperty.getSuid()==0) {
            ...
            try {
                ...
                if (imageList.size()>0) {
                    logger.debug("ImageSearchService.getImageQuery: found ImageNumber matching {"+keywordSearchProperty.getKeywords()+"}, count: "+imageList.size());
    ...
    public SearchResult getOrphanedQuery(ISearchProperty searchProperty, int page, int itemsPerPage) {
        ...
        try {
            //if (imageVersionSearchProperty.getPeriod()==ImageVersionSearchProperty.PERIOD_NONE) {
    ...
    public SearchResult getAdvancedImageQuery(ImageVersionSearchProperty imageVersionSearchProperty, int page, int itemsPerPage, User user) {

        SqlMapClient smc =AppSqlMap.getSqlMapInstance();
        SearchResult searchResult = new SearchResult();
        imageVersionSearchProperty.setUsedLanguage(getUsedLanguage());

        if (imageVersionSearchProperty.getSuid()==0) {
            //prepare statement
            imageVersionSearchProperty.setSuid(
                this.getUniqueId()
            );
            //if (Config.useAcl & Config.aclOnlyShowPermittetObjects) {
            if (true) {
                imageVersionSearchProperty.setGroupPrincipal(user.getSecurityGroup());
                imageVersionSearchProperty.setUserPrincipal(user.getUserId());
            }
            //prüfen auf imagenumber (wenn imagenumber vorkommt, diese(s) bild(er) zurückgeben
            try {
                long start = System.currentTimeMillis();
                logger.debug("{QUERY-TIME SearchService->getImageNumberQuery} start "+start);
                List imageList = smc.queryForList("getImageNumberQuery",imageVersionSearchProperty);
                logger.debug("{QUERY-TIME SearchService->getImageNumberQuery} finish ["+(System.currentTimeMillis()-start)+" ms]");
                if (imageList.size()>0) {
                    logger.debug("ImageSearchService.getImageQuery: found ImageNumber matching {"+imageVersionSearchProperty.getKeywords()+"}, count: "+imageList.size());
                    logger.debug("{QUERY-TIME SearchService->prepareImageNumberQuery} start "+start);
                    smc.update("prepareImageNumberQuery",imageVersionSearchProperty);
                    logger.debug("{QUERY-TIME SearchService->prepareImageNumberQuery} finish ["+(System.currentTimeMillis()-start)+" ms]");
                }
            } catch (SQLException e) {
                e.printStackTrace();
            ...
            //abfrage ausführen
            try {
                smc.update("prepareAdvancedImageQuery",imageVersionSearchProperty);
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
        ...
        if (page!=-1) {
            searchResult.setThisPage(page);
            searchResult.setSearchProperty(imageVersionSearchProperty);
            searchResult.setItemsPerPage(itemsPerPage);
            searchResult.setResultCount(getImageQueryCount(imageVersionSearchProperty));
            searchResult.setPageCount(
                    //anzahl der Seiten ausrechnen
                    (searchResult.getResultCount()%itemsPerPage > 0) ? (searchResult.getResultCount()/itemsPerPage) + 1 : searchResult.getResultCount()/itemsPerPage
            );

            imageVersionSearchProperty.setStartItem((page)*itemsPerPage);
            imageVersionSearchProperty.setItemCount(itemsPerPage);
        } else {
            imageVersionSearchProperty.setItemCount(-1);
        }
        ...
        try {
            //if (imageVersionSearchProperty.getPeriod()==ImageVersionSearchProperty.PERIOD_NONE) {
                ...
                imageList = smc.queryForList("getImageQuery",imageVersionSearchProperty);
            //} else {
            //    imageList = smc.queryForList("getAdvancedImagePeriodQuery",imageVersionSearchProperty);
            //}
        } catch (SQLException e) {
