 ...
 */
public class LightboxService extends MultiLanguageService {
     ...
     * @param imagesPerPage
     * @return PaginatedList mit den Bilder der Lightbox
     * @deprecated stattdessen ist die neue Multi-Language-Methode zu verwenden {@link LightboxService#getLightboxImagesPaginatedList(com.stumpner.mediadesk.core.database.sc.loader.SimpleLoaderClass, int)}
     */
    ...
    public boolean isInFav(int userId, int ivid) {
        ...
        List<ImageVersionMultiLang> imageList = getLightboxImageList(userId);

        for (ImageVersionMultiLang i : imageList) {
