...
public class ImageMetadataService {
    ...
    public ImageVersionMetadata getImageVersionMetadata(int ivid) {

        ImageVersionService imageService = new ImageVersionService();
        ImageVersionMetadata imageVersionMetadata = new ImageVersionMetadata();
        ...
        if (imageVersion!=null) {
            imageVersionMetadata.setImageVersion(imageVersion);
            imageVersionMetadata.setMetadata(imageMetadataService.getMetadata(ivid));



            //Ersteller-User laden:
            if (imageVersionMetadata.getImageVersion().getCreatorUserId()!=-1) {
                UserService userService = new UserService();
                try {
                    imageVersionMetadata.setCreator(
                        (User)userService.getById(imageVersionMetadata.getImageVersion().getCreatorUserId())
                    );
            ...
            if (imageVersion.getMayorMime().equalsIgnoreCase("text")) {
                ...
                try {
                    ...
                    imageVersionMetadata.setContent(content);
                } catch (IOException e) {
                    e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
                }
            }
