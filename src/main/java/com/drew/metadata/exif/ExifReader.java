...
public class ExifReader implements MetadataReader
{
    ...
    public ExifReader(byte[] data, String encoding)
    {
        _data = data;
    ...
    public Metadata extract(Metadata metadata)
    {
        ...
        // after the extraction validateFile, if we have the correct tags, we may be able to store thumbnail information
    ...
    private void processMakerNote(int subdirOffset, HashMap processedDirectoryOffsets, int tiffHeaderOffset)
    {
        ...
        else if ("KC".equals(firstTwoChars) || "MINOL".equals(firstFiveChars) || "MLY".equals(firstThreeChars) || "+M+M+M+M".equals(firstEightChars))
        {
            ...
            // TODO determine how to validateFile the information described at the above website
    ...
    private void processTag(Directory directory, int tagType, int tagValueOffset, int componentCount, int formatCode)
    {
        ...
        switch (formatCode)
        {
            case FMT_UNDEFINED:
                ...
                if (tagType==37510) {
                    //comment-Tag!! changed by franz stumpner
                    try {
                        directory.setString(tagType,new String(tagBytes,encoding));
    ...
    private String readString(int offset, int maxLength)
    {
        ...
        if (encoding==null) {
            ...
            try {
                //System.out.println("readString: "+new String(_data, offset, length,encoding));
                return new String(_data, offset, length,encoding);
            } catch (UnsupportedEncodingException e) {
                System.out.println("Probleme beim Encoding");
                return new String(_data, offset, length);
            }
        }
    }
