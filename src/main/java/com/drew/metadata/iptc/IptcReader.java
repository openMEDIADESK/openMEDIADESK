...
*/
     ...
     * @param encoding
     */
    public IptcReader(byte[] data, String encoding)
    {
        _data = data;
    ...
    private void processTag(Directory directory, int directoryType, int tagType, int offset, int tagByteCount)
    {
        ...
        if (tagByteCount < 1) {
            ...
            if (encoding==null) {
                ...
                //use spcified encoding (by franz stumpner)
                try {
                    //System.out.println("Read String default encoded: "+new String(_data, offset, tagByteCount));
                    //System.out.println("Read String "+encoding+" encoded: "+new String(_data, offset, tagByteCount,encoding));

                    str = new String(_data, offset, tagByteCount,encoding);
                } catch (UnsupportedEncodingException e) {
                    System.out.println("encoding not supported: "+encoding+", use standard os...");
                    str = new String(_data, offset, tagByteCount);
                }
            }
        }
