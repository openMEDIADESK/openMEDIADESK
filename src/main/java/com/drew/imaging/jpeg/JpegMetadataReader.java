...
//    public static Metadata readMetadata(ImageReader reader) throws JpegProcessingException{}
    ...
    public static Metadata readMetadata(InputStream in) throws JpegProcessingException
    {
        ...
        return extractMetadataFromJpegSegmentReader(segmentReader,null);
    }
    ...
    /**
     * Function allows to use the specified File-Encoding istead the
     * default encoding of the OS when reading Metadata
     * Added by franz stumpner
     * @param file
     * @param encoding
     * @return Metadata Class containing the Metadata
     */
    public static Metadata readMetadata(File file,String encoding) throws JpegProcessingException {

        JpegSegmentReader segmentReader = new JpegSegmentReader(file);
        return extractMetadataFromJpegSegmentReader(segmentReader,encoding);

    }

    public static Metadata extractMetadataFromJpegSegmentReader(JpegSegmentReader segmentReader, String encoding)
    {
        if (encoding==null) {
            encoding = System.getProperty("file.encoding");
            //System.out.println("no Encoding given, using default: "+encoding);
        }

        final Metadata metadata = new Metadata();
        try {
            byte[] exifSegment = segmentReader.readSegment(JpegSegmentReader.SEGMENT_APP1);
            new ExifReader(exifSegment,encoding).extract(metadata);
        ...
        try {
            ...
            //use the specified encoding - changed by franz stumpner
            new IptcReader(iptcSegment,encoding).extract(metadata);
        } catch (JpegProcessingException e) {
            e.printStackTrace();
		...
		try {
			...
			new JpegCommentReader(jpegCommentSegment,encoding).extract(metadata);
		} catch (JpegProcessingException e) {
            e.printStackTrace();
