...
<ol class="breadcrumb">
    ...
    <li class="active"><i class="fa fa-cogs fa-fw"></i> <spring:message code="set.web"/></li>
</ol>
<!-- /breadcrumbs -->
<!-- ordnertitel und infos -->
<h3><spring:message code="set.web"/></h3>
...
<div class="row">
    ...
    <form method="post" action="<c:url value="setweb"/>">
    ...
    <spring:bind path="command.title">
        ...
        <label for="input<c:out value="${status.expression}"/>"><spring:message code="set.web.title"/></label>
    ...
    <spring:bind path="command.copyright">
        <div class="form-group<c:if test="${status.error}"> has-error has-feedback</c:if>">
        <label for="input<c:out value="${status.expression}"/>"><spring:message code="set.web.copyright"/></label>
        <input type="text" class="form-control input-sm" id="input<c:out value="${status.expression}"/>" name="<c:out value="${status.expression}"/>" value="<c:out value="${status.value}"/>" <c:if test="${status.error}"> aria-describedby="eingabefeldFehler2Status"</c:if>>
        <c:if test="${status.error}">
          <span class="glyphicon glyphicon-remove form-control-feedback" aria-hidden="true"></span>
          <span id="eingabefeldFehler2Status" class="sr-only">(Fehler)</span>
    ...
    <spring:bind path="command.corporateSiteName">
        <div class="form-group<c:if test="${status.error}"> has-error has-feedback</c:if>">
        <label for="input<c:out value="${status.expression}"/>"><spring:message code="set.web.corporateSiteName"/></label>
        <input type="text" class="form-control input-sm" id="input<c:out value="${status.expression}"/>" name="<c:out value="${status.expression}"/>" value="<c:out value="${status.value}"/>" <c:if test="${status.error}"> aria-describedby="eingabefeldFehler2Status"</c:if>>
        <c:if test="${status.error}">
          <span class="glyphicon glyphicon-remove form-control-feedback" aria-hidden="true"></span>
          <span id="eingabefeldFehler2Status" class="sr-only">(Fehler)</span>
    ...
    <spring:bind path="command.corporateSiteLink">
        <div class="form-group<c:if test="${status.error}"> has-error has-feedback</c:if>">
        <label for="input<c:out value="${status.expression}"/>"><spring:message code="set.web.corporateSiteLink"/></label>
        <input type="url" class="form-control input-sm" id="input<c:out value="${status.expression}"/>" name="<c:out value="${status.expression}"/>" value="<c:out value="${status.value}"/>" <c:if test="${status.error}"> aria-describedby="eingabefeldFehler2Status"</c:if>>
        <c:if test="${status.error}">
          <span class="glyphicon glyphicon-remove form-control-feedback" aria-hidden="true"></span>
          <span id="eingabefeldFehler2Status" class="sr-only">(Fehler)</span>
        </c:if>
    ...
    <button type="submit" name="Submit" class="btn btn-default"><spring:message code="folderedit.submit"/></button>
